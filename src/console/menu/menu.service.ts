...
export class MenuService {
  ...
  private convert2StoreObj = async (store_id: string): Promise<Store> => {
    const RealStoreId = Number(
      Buffer.from(store_id, "base64").toString("binary")
    );
    const store = getRepository(Store);
    return await store.findOne(RealStoreId);
  };

  private convert2CategoryObj = async (
    category: number[]
  ): Promise<Category[]> => {
  ...
  private convert2OptionGroupObj = async (
    optionGroup: number[]
  ): Promise<OptionGroup[]> => {
  ...
  private convert2EnableTimeObj = async (
    Enabletime: number
  ): Promise<EnableTime> => {
  ...
  private menuCreateDtoToEntity = async (
    dto: MenuCreateDto,
    storeId: string
  ): Promise<Menu> => {
    const menu = new Menu();
...
>>>>>>> 3653f2b12519c91cba985b2a5626e01dd58d560f
  ...
  async getMenuInfo(menuId: number): Promise<MenuInfoResponseDto> {
    const menu = await this.menuRepository.findOne(menuId, {
      relations: ["store_id", "categories", "optionGroups", "enable_time"],
    });
    if (!!menu) {
      return new MenuInfoResponseDto(menu);
    } else {
...
>>>>>>> 3653f2b12519c91cba985b2a5626e01dd58d560f
  ...
  async removeEnableTimeInMenu(menuId: number): Promise<BasicMessageDto> {
    const menu = await this.menuRepository.findOne(menuId);
